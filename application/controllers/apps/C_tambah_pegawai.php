...
class C_tambah_pegawai extends CI_Controller {
	var $cekLog;
	...
	function cekAccess($action){
		return $this->m_security->cekAkses(22,$action);
	}
	...
	function views(){
		...
		$koordinat = $this->input->get('group') ? $this->input->get('group') : 'berita';
		...
		$tot_hal = $this->m_pegawai->views("","",$sidx,$sord,$where);
	...
	function rows(){
		...
		$tot_hal = $this->m_pegawai->views("","","","",$where);
	...
	function index(){
		$menudata="apps/home, home, / |,user,";
		$data["breadcrumb"]= data_breadcrumb($menudata);
		$data["url_link"] = 'apps/c_tambah_pegawai';
		$data["tab"] = "tab1";
		$data["form"] = "add";
		$data["op"] = "add";
	...
	function delete(){
		...
		if($pilih!=""){
			...
			foreach ($pilih as $key) {
				...
				$ket_log .="Grup : ".$data_a['koordinat']."<br>";
				$ket_log .="email : ".$data_a['email']."<br>";
				$log = datalog(
					'DELETE',
